...
import { createPrivateWallet, loadPrivateWallet } from "@/lib/railgun/wallet-actions";
...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    setStatusMsg("⏳ 正在登入...");
    try {
      await loadPrivateWallet(loginPassword);
      setStatusMsg("✅ 登入成功！");
      // 登入成功後跳轉到主頁或 Dashboard (這裡先示範跳轉到 cross-chain)
  ...
  const handleCreateOrImport = async () => {
    ...
    try {
      await createPrivateWallet(importPassword, importMnemonic);
      setStatusMsg("✅ 錢包創建/匯入成功！");
      router.push("/cross-chain");
    } catch (error: any) {
      console.error(error);
